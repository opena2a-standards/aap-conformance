...
The pair is Node + Python deliberately: the AAP reference broker (Secretless)
is TypeScript, so the Node verifier exercises the same JOSE primitives the
reference mints with, and the Python verifier mirrors the spec repo's
generator. Both implement the same pinned check order; the
duplicate-header-member fixture exists precisely to catch strict-vs-last-wins
JSON parsing divergence between them (the atx-conformance Go/Python lesson).
...
VERIFIERS = {
    "node": {
        "cwd": REPO_ROOT / "verifiers" / "node",
        "cmd": ["node", "verify.mjs", "../../fixtures"],
    },
    "python": {
